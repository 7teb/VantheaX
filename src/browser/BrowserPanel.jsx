...
const BrowserPanel = ({ api, controller, full, open, labels, onToggleFull, onClose }) => {
  ...
  const loadTab = (id, value) => {
    ...
    return controller.loadTab(id, url);
  };
  ...
  const handleUrlKeyDown = (event) => {
    if (event.key === "Escape" && activeTab) {
      controller.patchTab(activeTab.id, { pendingUrl: null });
  ...
  return (
    <aside className="browser-panel">
      <div className="browser-head">
        <div className="browser-tabs">
          {state.tabs.map((tab, index) => (
            <div key={tab.id} className={tab.id === state.activeId ? "browser-tab is-active" : "browser-tab"} onMouseDown={() => controller.selectTab(tab.id)}>
          ...
          <button className="browser-add" title={labels.addTab} onClick={() => controller.addTab()}><Plus size={14} /></button>
        </div>
      ...
      <form className="browser-toolbar" onSubmit={submitUrl}>
        <button type="button" className="browser-nav" disabled={!activeTab?.canBack} title={labels.back} onClick={() => controller.runGuestAction(state.activeId, (guest) => guest.goBack())}><ChevronLeft size={16} /></button>
        <button type="button" className="browser-nav" disabled={!activeTab?.canForward} title={labels.forward} onClick={() => controller.runGuestAction(state.activeId, (guest) => guest.goForward())}><ChevronRight size={16} /></button>
        <input
          ref={urlInputRef}
          ...
          onChange={(event) => activeTab && controller.patchTab(activeTab.id, { pendingUrl: event.target.value })}
          onKeyDown={handleUrlKeyDown}
        />
        <button type="button" className={activeTab?.loading ? "browser-nav browser-reload is-loading" : "browser-nav browser-reload"} title={labels.reload} onClick={() => controller.runGuestAction(state.activeId, (guest) => guest.reload())}><RotateCw size={15} /></button>
      </form>
      <div className="browser-views">
        {state.tabs.map((tab) => (
          <BrowserView
            ...
            onPatch={controller.patchTab}
            onAttached={controller.attachGuest}
            onDetached={controller.detachGuest}
